...
def ability(
    name: str, description: str, parameters: List[AbilityParameter], 
    output_type: str, category: Set[AbilityCategory] = {AbilityCategory.GENERAL}
):
    def decorator(func):
...
class AbilityRegister:
    ...
    def list_abilities_by_category(self, category: AbilityCategory) -> List[Ability]:
        return [ability for ability in self.abilities.values() if category in ability.category]

    def list_abilities_by_category_for_prompt(self, category: AbilityCategory) -> List[str]:
        return [str(ability) for ability in self.list_abilities_by_category(category)]

    def abilities_description(self) -> str:
        abilities_by_category = {}
        for ability in self.abilities.values():
            for category in ability.category:
                # Initialize category in the dictionary if not present
                if category not in abilities_by_category:
                    abilities_by_category[category] = []
                abilities_by_category[category].append(str(ability))

        abilities_description = ""
        for category, abilities in abilities_by_category.items():
            if abilities_description != "":
                abilities_description += "\n"
            abilities_description += f"{category.value}:"
            for ability in abilities:
                abilities_description += f"\n  {ability}"
...
if __name__ == "__main__":
    ...
    sys.path.append("/Users/vaishnav/dev/prometheus")
    register = AbilityRegister(agent=None)
    print(register.abilities_description())
    # async def main():
    #     result = await register.run_ability("abc", "list_files", "/Users/vaishnav/dev/prometheus")
    #     print(result)
